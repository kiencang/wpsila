...
# Tạo tên Database ngẫu nhiên: bắt đầu bằng wp_ + 8 ký tự thường/số
GEN_DB_NAME="wp_$(cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 8 | head -n 1)"

# Tạo tên User ngẫu nhiên: bắt đầu bằng user_ + 8 ký tự thường/số
GEN_DB_USER="user_$(cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 8 | head -n 1)"

# Tạo Password ngẫu nhiên: 20 ký tự (Hoa + Thường + Số), loại bỏ ký tự đặc biệt để an toàn tuyệt đối cho SQL
GEN_DB_PASS="$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 20 | head -n 1)"
...
CRED_FILE="$HOME/wp_credentials.txt"
cat > "$CRED_FILE" <<EOF
----------------------------------------
